...
into your current R session. [r2dd.data](https://2degreesinvesting.github.io/r2dii.data) 
includes fake data to help demonstrate the tool and 
[r2dd.match](https://2degreesinvesting.github.io/r2dii.match) provides functions 
...
See [r2dd.match](https://2degreesinvesting.github.io/r2dii.match) for a more 
...
The [Market Share Approach](articles/target-market-share.html) is used to 
...
The [Sectoral Decarbonization Approach](articles/target-market-share.html) is 
used to calculate scenario targets for the `emission_factor` of a sector. For 
example, we can use this approach to set targets for the average emission factor 
...
of `target_market_share()` and `target_sda()`. We will highlight a couple of 
common plots. 
...
market_share_targets_portfolio %>% 
  filter(sector == "power",
         ...
         year == max(year)) %>% # different targets can be plotted by setting these filters
  group_by(technology) %>% 
...
renewables_data <- filter(market_share_targets_portfolio, 
                          sector == "power",
...
just_targets %>% 
  ...
  geom_line(data = just_data, aes(x = year, 
                                  y = weighted_production_value, 
