...
test_results <- read_xlsx("single_indicator_sample_results.xlsx", sheet = 1)
test_audit <- read_xlsx("single_indicator_sample_audit.xlsx", sheet = 1)
scenarios <- read_csv("scenario_relationships.csv")
sector_weightings <- read_csv("tech_sector_weighting.csv")

singel_indicator <- function(input_results = test_results, upper_temp_threshold = 10, lower_temp_threshold = 1, start_year = START.YEAR, allocation = "PortfolioWeight") {
  ...
  temp <- input_results %>% 
    filter(Allocation == allocation & 
             ...
             Year >= START.YEAR & Year <= START.YEAR+5 & 
  ...
  temp <- scenario_relationships %>%
    pivot_wider(
      names_from = relation, values_from = c(scen_tech_prod, temp)
      ) %>% #spreading out the different relations. 
    inner_join(
      temp, by = c("Sector", "Technology", "asset_class", "Investor.Name", "Portfolio.Name")
      ) %>% 
    distinct(
      Investor.Name, Portfolio.Name, Sector, Technology, asset_class, temp_lower, temp_reference, temp_upper, scen_tech_prod_lower, scen_tech_prod_reference, scen_tech_prod_upper, plan_tech_prod
      )
  ...
  temp <- temp %>% 
    group_by(Sector) %>% 
    mutate(temp_lower_range = temp_reference - temp_lower,
  ...
  temp <- temp %>% 
    mutate(
      scen_tech_prod_reference = case_when(
        scen_tech_prod_reference_temp > scen_tech_prod_lower_temp & Technology %not in% brown_technologies ~ scen_tech_prod_lower_temp)
      )
  
  temp <- temp %>% 
    mutate(
      scen_tech_prod_lower = case_when(
        scen_tech_prod_reference_temp > scen_tech_prod_lower_temp & Technology %not in% brown_technologies ~ scen_tech_prod_reference_temp)
    )
  ...
  temp <- scenario_port_relation(input = temp, 
                                 metric_name = "factor", 
...
temp <- singel_indicator(input_results = test_results,
