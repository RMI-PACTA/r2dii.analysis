...
goals. They summarize the production of a financial portfolio and calculate
targets for a given climate scenario. These tools implement in R the last step
of the free software PACTA (Paris Agreement Capital Transition Assessment; <https://2degrees-investing.org/>). Financial institutions use PACTA to
...
* Use `summarize_company_production()` then `target_market_share_company()` to calculate scenario-targets for each company.
...
* Use `summarize_portfolio_production()` then `target_market_share_portfolio()` to calculate scenario-targets for the whole portfolio:
