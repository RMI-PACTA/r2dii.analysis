...
and analysis tools commonly used for climate scenario analysis. For more information visit <https://2degrees-investing.org/>.
...
* Use `summarize_company_production()` then `add_company_target()` to calculate senario-targets for each company.
...
* Use `summarize_portfolio_production()` then `add_portfolio_target()` to calculate senario-targets for the whole portfolio:
