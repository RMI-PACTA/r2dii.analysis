...
# This code in a separate chunk run slowly or failed to run
source("setup.R")

# Reuse code in setup.R
show_setup <- function(path = "setup.R") {
  lines <- readLines(path, encoding = "UTF-8")
  writeLines(r_chunk(lines))
}

r_chunk <- function(x) c("```r\n", x, "\n```")
```
...
```{r unlabeled-75, exercise=TRUE, exercise.eval=TRUE, exercise.setup='setup'}
matched %>%
  target_sda(
    ald = ald,
...
```{r unlabeled-85, exercise=TRUE, exercise.eval=TRUE, exercise.setup='setup'}
matched %>%
  target_market_share(
    ald = ald,
...
```{r unlabeled-96, exercise=TRUE, exercise.eval=TRUE, exercise.setup='setup'}
matched %>%
  target_market_share(
    ald = ald,
    scenario = scenario_demo_2020,
...
```{r joint, exercise=TRUE, exercise.eval=TRUE, exercise.setup='setup'}
...
```{r portfolio-level, exercise=TRUE, exercise.eval=TRUE, exercise.setup='joint'}
# portfolio level
loanbook_joined_to_ald_scenario %>%
  summarize_weighted_production(scenario, tmsr, smsp, region)
```

```{r company-level, exercise=TRUE, exercise.eval=TRUE, exercise.setup='joint'}
# company level
loanbook_joined_to_ald_scenario %>%
