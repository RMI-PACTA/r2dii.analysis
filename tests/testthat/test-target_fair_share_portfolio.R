test_that("with bad `data` errors with informative message", {
  expect_error(target_fair_share_portfolio("bad"), "data.frame.*not.*TRUE")
})
...
test_that("with fake data outputs known value", {
  ...
  expect_known_value(out, "ref-target_fair_share_portfolio", update = FALSE)
})
...
test_that("with data lacking crucial columns errors with informative message", {
  expect_error_missing_names <- function(name) {
    ...
    expect_error(
      class = "missing_names",
      target_fair_share_portfolio(data)
    )
  }
...
test_that("with known input outputs as expected", {
  ...
  out1 <- target_fair_share_portfolio(data)
