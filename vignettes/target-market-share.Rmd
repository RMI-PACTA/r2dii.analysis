...
The goal of this article is to define _market-share_ and to show how to calculate the market-share target for a given scenario.

## Scenario market-shares

You want to study how a portfolio would perform in a specific climate scenario. 

How can you allocate [scenario efforts] to the production profile of your portfolio? You can do it by technology and by sector.
...
* Create a "master" dataset: Joining the `matched` dataset to three other datasets:

    1. The asset-level data (`ald`);
    2. A dataset giving the production `scenario`;
    3. A dataset mapping countries to `regions`.
...
* Calculate market-share targets at the company level.
