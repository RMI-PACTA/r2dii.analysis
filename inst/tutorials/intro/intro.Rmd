...
```{r unlabeled-85, exercise.setup='setup'}
matched %>%
  target_market_share(
    ald = ald,
...
```{r unlabeled-96, exercise.setup='setup'}
matched %>%
  target_market_share(
    ald = ald,
    scenario = scenario_demo_2020,
...
```{r portfolio-level, exercise.setup='joint'}
# portfolio level
loanbook_joined_to_ald_scenario %>%
  summarize_weighted_production(scenario, tmsr, smsp, region)
```

```{r company-level, exercise.setup='joint'}
# company level
loanbook_joined_to_ald_scenario %>%
